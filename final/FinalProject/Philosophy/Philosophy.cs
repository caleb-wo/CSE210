...
namespace Philosophies 
{
    class Philosophy
    {
        ...
        public string ToCsv()
        {
            return $"{GetPhilosophy()}<><>{GetPurpose()}<><>{GetSlogan()}<><>{GetCompleted()}<><>{GetDateStarted()}";
        }

        public void CompletePhilosophy()
        {
            System.Console.WriteLine("Well done. We hope this philosophy inspired much success!");
            _IsComplete = true;
        }
    }
}
